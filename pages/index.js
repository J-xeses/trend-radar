...
const WorkflowStep = ({ num, label, icon, active, done }) => (
  <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:4, flex:1 }}>
    <div style={{
      width:36, height:36, borderRadius:10,
      background: done ? T.gd : active ? T.acd : T.c,
      border: `2px solid ${done ? T.g : active ? T.ac : T.ba}`,
...
const Arrow = () => (
  <div style={{ color:T.tm, fontSize:14, paddingBottom:16, flexShrink:0 }}>›</div>
);
...
const ApiKeyModal = ({ onSave, onSkip }) => {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"#00000090", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
      <div style={{ background:T.s, border:`1px solid ${T.ba}`, borderRadius:16, padding:28, maxWidth:420, width:"100%", animation:"fadeUp .3s ease" }}>
        <div style={{ fontSize:28, marginBottom:8, textAlign:"center" }}>🔑</div>
        <div style={{ fontSize:18, fontWeight:800, marginBottom:6, textAlign:"center" }}>Anthropic API Key 설정</div>
        <div style={{ fontSize:13, color:T.ts, lineHeight:1.7, marginBottom:20, textAlign:"center" }}>
          트렌드 분석 및 대본 생성에 Claude API가 필요합니다.<br/>
          <span style={{ color:T.am }}>키는 브라우저에만 저장되며 외부로 전송되지 않습니다.</span>
        </div>
        <input
          value={key}
          ...
          placeholder="sk-ant-..."
          type="password"
          style={{ width:"100%", background:T.c, border:`1px solid ${T.ba}`, borderRadius:9, padding:"12px 14px", color:T.t, fontSize:13, fontFamily:T.m, marginBottom:12, boxSizing:"border-box" }}
        />
        <div style={{ display:"flex", gap:8 }}>
          <button onClick={onSkip} style={{ flex:1, padding:11, borderRadius:9, background:"transparent", border:`1px solid ${T.ba}`, color:T.ts, fontSize:13, fontWeight:600, cursor:"pointer" }}>
            나중에
          </button>
          <button onClick={() => key.startsWith("sk-") && onSave(key)} style={{ flex:2, padding:11, borderRadius:9, background:key.startsWith("sk-") ? T.ac : T.c, border:"none", color: key.startsWith("sk-") ? "#fff" : T.tm, fontSize:13, fontWeight:700, cursor: key.startsWith("sk-") ? "pointer" : "not-allowed" }}>
            저장 후 시작
          </button>
        </div>
        <div style={{ fontSize:11, color:T.tm, textAlign:"center", marginTop:12 }}>
          API Key가 없으면? →{" "}
          <a href="https://console.anthropic.com" target="_blank" rel="noreferrer" style={{ color:T.ac }}>console.anthropic.com</a>
        </div>
      </div>
    </div>
  );
...
export default function TrendRadarV5() {
  ...
  const [apiKey, setApiKey] = useState(() => typeof window !== "undefined" ? localStorage.getItem("tr_apikey") || "" : "");
  const [showApiModal, setShowApiModal] = useState(false);
  const [workflowStep, setWorkflowStep] = useState(0); // 0=idle,1=collected,2=analyzing,3=scripting,4=pipeline
  ...
  const analyze = useCallback(async (trend) => {
    ...
    setSel(trend); setAL(true); setAnalysis(null); setScript(null);
    setWorkflowStep(2);
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role:"user", content:`Korean YouTube strategist for "AI × 자기계발" channel targeting 20-40대 직장인.
...
{"opportunity":<1-10>,"competition":"<낮음|보통|높음>","urgency":"<즉시|1주내|여유>","summary_ko":"<Korean 2-3 sentences>","angle":"<unique channel angle Korean>","videos":[{"title":"<Korean max 40ch>","format":"<숏폼|롱폼|튜토리얼|분석>","views":"<예상>"},{"title":"...","format":"...","views":"..."},{"title":"...","format":"...","views":"..."}],"tags":["#t1","#t2","#t3","#t4","#t5"],"best_time":"<Korean>"}`}]
        })
      });
      const d = await res.json();
      const raw = (d.content || []).map(c2 => c2.text || "").join("");
  ...
  const genScript = useCallback(async (v) => {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          model: MODEL, max_tokens: 1200,
          messages: [{ role:"user", content:`Korean YT scriptwriter. Script for:"${v.title}" (${v.format}) RESPOND ONLY IN VALID JSON.
{"hook":"<15sec Korean>","sections":[{"ts":"0:00","name":"후킹","desc":"<KR>"},{"ts":"0:15","name":"문제제기","desc":"<KR>"},{"ts":"2:00","name":"핵심","desc":"<KR>"},{"ts":"6:00","name":"실전","desc":"<KR>"},{"ts":"8:00","name":"CTA","desc":"<KR>"}],"seo":["t1","t2","t3","t4","t5"],"desc":"<2-sentence KR>"}`}]
        })
      });
      const d = await res.json();
      const raw = (d.content || []).map(c2 => c2.text || "").join("");
      setScript(JSON.parse(raw.replace(/```json|```/g, "").trim()));
    } catch { setScript({ error: true }); }
    setSL(false);
  }, [apiKey]);
  ...
  const saveApiKey = (key) => {
    localStorage.setItem("tr_apikey", key);
    setApiKey(key);
    ...
    <div style={{ minHeight:"100vh", background:T.bg, color:T.t, fontFamily:T.f }}>
      ...
      <header style={{ padding:"14px 20px", borderBottom:`1px solid ${T.b}`, background:`${T.s}ee`, backdropFilter:"blur(12px)", display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:100 }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          <div style={{ width:36, height:36, borderRadius:10, background:`linear-gradient(135deg,${T.ac},${T.cy})`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:16, fontWeight:900, color:"#fff", boxShadow:`0 0 16px ${T.ac}40` }}>T</div>
          <div>
      ...
      <div style={{ padding:"14px 20px", background:T.s, borderBottom:`1px solid ${T.b}` }}>
        <div style={{ fontSize:10, fontFamily:T.m, color:T.tm, marginBottom:10, fontWeight:600, letterSpacing:".08em" }}>WORKFLOW</div>
        <div style={{ display:"flex", alignItems:"center", gap:0 }}>
          <WorkflowStep num={1} label="수집" icon="📡" active={workflowStep===1} done={workflowStep>1} />
          <Arrow />
          <WorkflowStep num={2} label="트렌드선택" icon="🔍" active={workflowStep===1} done={workflowStep>2} />
          <Arrow />
          <WorkflowStep num={3} label="AI분석" icon="🤖" active={workflowStep===2} done={workflowStep>2} />
          <Arrow />
          <WorkflowStep num={4} label="대본생성" icon="✍️" active={workflowStep===3} done={workflowStep>3} />
          <Arrow />
          <WorkflowStep num={5} label="파이프라인" icon="📋" active={workflowStep===4} done={false} />
        </div>
        {workflowStep === 0 && <div style={{ fontSize:11, color:T.ts, marginTop:8, textAlign:"center" }}>데이터를 수집하면 워크플로우가 시작됩니다</div>}
        {workflowStep === 1 && <div style={{ fontSize:11, color:T.am, marginTop:8, textAlign:"center" }}>👆 아래 트렌드 카드를 클릭하면 AI 분석이 시작됩니다</div>}
        {workflowStep === 2 && <div style={{ fontSize:11, color:T.ac, marginTop:8, textAlign:"center", animation:"pulse 1.5s infinite" }}>🤖 Claude가 콘텐츠 기회를 분석하고 있습니다...</div>}
        {workflowStep === 3 && <div style={{ fontSize:11, color:T.g, marginTop:8, textAlign:"center" }}>✅ 분석 완료! 영상 아이디어에서 대본을 생성하거나 파이프라인에 추가하세요</div>}
        {workflowStep === 4 && <div style={{ fontSize:11, color:T.g, marginTop:8, textAlign:"center" }}>🎬 파이프라인에 추가됐어요! 파이프라인 탭에서 제작 단계를 관리하세요</div>}
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(8,1fr)", borderBottom:`1px solid ${T.b}`, background:T.s2 }}>
        {Object.entries(SRC).map(([k, v]) => {
          const st2 = fStatus[k];
          return (
            <div key={k} onClick={() => setSrcF(srcF === k ? "all" : k)} style={{ padding:"10px 4px", borderRight:`1px solid ${T.b}`, textAlign:"center", cursor:"pointer", background: srcF===k ? v.b : "transparent", transition:"all .2s" }}>
              <div style={{ fontSize:18 }}>{v.i}</div>
              <div style={{ fontSize:15, fontFamily:T.m, fontWeight:800, color:st2?.ok?T.g:loading?T.am:T.r, marginTop:2 }}>{st2 ? st2.n : "—"}</div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", borderBottom:`1px solid ${T.b}` }}>
        {[
          ...
          { l:"🔥 폭발 트렌드", v:stats.exp, c:T.r },
          { l:"📈 상승 트렌드", v:stats.rise, c:T.am },
      ...
      <div style={{ display:"flex", borderBottom:`1px solid ${T.b}`, background:T.s }}>
        {[{ id:"trends", l:"🔍 라이브 트렌드", n:filtered.length }, { id:"pipeline", l:"📋 파이프라인", n:pipe.length }].map(tb => (
          <button key={tb.id} onClick={() => setTab(tb.id)} style={{ flex:1, padding:"14px", background:"transparent", border:"none", borderBottom:tab===tb.id?`2px solid ${T.ac}`:"2px solid transparent", color:tab===tb.id?T.t:T.tm, fontSize:14, fontWeight:700, cursor:"pointer", transition:"all .2s" }}>
      ...
      {tab === "trends" && <div>
        ...
        <div style={{ padding:"10px 16px", borderBottom:`1px solid ${T.b}` }}>
          <input value={q} onChange={e => setQ(e.target.value)} placeholder="🔍 키워드 검색..." style={{ width:"100%", background:T.c, border:`1px solid ${T.b}`, borderRadius:9, padding:"10px 14px", color:T.t, fontSize:13, fontFamily:T.f }} />
        </div>
        <div style={{ display:"flex", gap:5, padding:"8px 16px", overflowX:"auto", borderBottom:`1px solid ${T.b}` }}>
          {CATS.map(c2 => (
            <button key={c2.id} onClick={() => setCat(c2.id)} style={{ padding:"5px 12px", borderRadius:16, fontSize:12, fontWeight:700, cursor:"pointer", border:"none", background:cat===c2.id?T.ac:T.c, color:cat===c2.id?"#fff":T.ts, whiteSpace:"nowrap", transition:"all .2s" }}>
              {c2.i} {c2.l}
            </button>
          ))}
        </div>
                ...
                <div style={{ display:"flex", gap:12, alignItems:"flex-start" }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <div style={{ display:"flex", gap:12, marginTop:6, fontSize:11, fontFamily:T.m, color:T.tm }}>
                      ...
                      <span style={{ marginLeft:"auto", color:T.ac, fontWeight:600 }}>클릭하여 AI 분석 →</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        {(aL || analysis) && sel && (
          <div ref={analysisRef} style={{ margin:"0 14px 14px", background:T.s2, border:`1px solid ${T.acb}`, borderRadius:14, overflow:"hidden", animation:"fadeUp .25s ease" }}>
            <div style={{ padding:"16px 18px", borderBottom:`1px solid ${T.b}`, display:"flex", alignItems:"center", gap:10, background:`linear-gradient(135deg,${T.acd},transparent)` }}>
              ...
              <div style={{ flex:1 }}>
                <div style={{ fontSize:14, fontWeight:800 }}>AI 콘텐츠 분석</div>
            ...
            {aL && (
              <div style={{ padding:48, textAlign:"center" }}>
                <Spin s={24} />
                <div style={{ marginTop:12, fontSize:13, color:T.ts, animation:"pulse 1.5s infinite" }}>Claude가 분석중...</div>
              </div>
            )}
            ...
            {analysis && !analysis.error && (
              <div style={{ padding:16 }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8, marginBottom:14 }}>
                  {[
                    { l:"기회점수", v:`${analysis.opportunity}/10`, c:T.ac },
                    { l:"경쟁강도", v:analysis.competition, c:T.t },
                    { l:"시급성", v:analysis.urgency, c:analysis.urgency==="즉시"?T.r:T.am },
                  ].map((s2, i) => (
                ...
                {analysis.summary_ko && (
                  <div style={{ background:T.c, borderRadius:10, padding:14, marginBottom:10 }}>
                    <div style={{ fontSize:10, color:T.ts, fontFamily:T.m, fontWeight:600, marginBottom:6 }}>📊 분석 요약</div>
                    <div style={{ fontSize:13, lineHeight:1.7 }}>{analysis.summary_ko}</div>
                  </div>
                )}

                {analysis.angle && (
                  <div style={{ background:`${T.ac}0c`, border:`1px solid ${T.acb}`, borderRadius:10, padding:12, marginBottom:10 }}>
                    <div style={{ fontSize:10, color:T.ac, fontFamily:T.m, fontWeight:700, marginBottom:4 }}>💡 채널 앵글</div>
                    <div style={{ fontSize:13, lineHeight:1.6 }}>{analysis.angle}</div>
                  </div>
                )}

                {analysis.best_time && (
                  <div style={{ background:T.amd, border:`1px solid ${T.amb}`, borderRadius:10, padding:12, marginBottom:14, display:"flex", gap:10, alignItems:"center" }}>
                    <span style={{ fontSize:18 }}>⏰</span>
                    <div>
                      <div style={{ fontSize:10, color:T.am, fontFamily:T.m, fontWeight:700 }}>업로드 타이밍</div>
                      <div style={{ fontSize:13, marginTop:2 }}>{analysis.best_time}</div>
                    </div>
                  </div>
                )}
                ...
                {(analysis.videos || []).map((v, i) => (
                  <div key={i} style={{ background:T.c, border:`1px solid ${T.b}`, borderRadius:10, padding:14, marginBottom:8 }}>
                    ...
                    <div style={{ display:"flex", gap:5, marginBottom:12 }}>
                      <Pill>{v.format}</Pill>
                      <Pill color={T.g}>예상 {v.views}</Pill>
                    </div>
                    <div style={{ display:"flex", gap:8 }}>
                      <button onClick={e => { e.stopPropagation(); addPipe(sel, v); }} style={{ flex:1, padding:"9px", borderRadius:8, background:T.acd, border:`1px solid ${T.acb}`, color:T.ac, fontSize:12, fontWeight:700, cursor:"pointer" }}>
                        📋 파이프라인에 추가
                      </button>
                      <button onClick={e => { e.stopPropagation(); genScript(v); }} style={{ flex:1, padding:"9px", borderRadius:8, background:T.gd, border:`1px solid ${T.gb}`, color:T.g, fontSize:12, fontWeight:700, cursor:"pointer" }}>
                        ✍️ 대본 생성
                      </button>
                    </div>
                  </div>
                ...
                {analysis.tags && (
                  <div style={{ display:"flex", gap:4, flexWrap:"wrap", marginTop:10 }}>
                    {analysis.tags.map((tg, i) => <Pill key={i} color={T.ac} bg={T.acd}>{tg}</Pill>)}
                  </div>
                )}
              </div>
            )}
            {analysis?.error && <div style={{ padding:24, textAlign:"center", color:T.r, fontSize:13 }}>분석 실패. API Key를 확인하거나 다시 시도해주세요.</div>}
          </div>
        )}
        ...
        {(sL || script) && (
          <div style={{ margin:"0 14px 14px", background:T.s2, border:`1px solid ${T.gb}`, borderRadius:14, overflow:"hidden", animation:"fadeUp .25s ease" }}>
            ...
            {script && !script.error && (
              <div style={{ padding:16 }}>
                {script.hook && (
                  <div style={{ background:`${T.ac}0c`, border:`1px solid ${T.acb}`, borderRadius:10, padding:14, marginBottom:12 }}>
                    <div style={{ fontSize:10, color:T.ac, fontFamily:T.m, fontWeight:700, marginBottom:6 }}>🎯 오프닝 후킹 (15초)</div>
                    <div style={{ fontSize:14, lineHeight:1.7, fontStyle:"italic", color:T.t }}>"{script.hook}"</div>
                  </div>
                )}
                ...
                {script.desc && (
                  <div style={{ background:T.c, borderRadius:9, padding:12, marginTop:10 }}>
                    <div style={{ fontSize:10, color:T.tm, fontFamily:T.m, fontWeight:600, marginBottom:4 }}>📝 설명문</div>
                    <div style={{ fontSize:12, lineHeight:1.6, color:T.ts }}>{script.desc}</div>
                  </div>
                )}
              </div>
            )}
            {script?.error && <div style={{ padding:24, textAlign:"center", color:T.r, fontSize:13 }}>대본 생성 실패. 다시 시도해주세요.</div>}
          </div>
        )}
      </div>}
          ...
          {!pipe.length ? (
            <div style={{ textAlign:"center", padding:60, color:T.tm }}>
              ...
              <div style={{ fontSize:13, color:T.ts, lineHeight:1.7 }}>
                트렌드 탭에서 관심 트렌드를 클릭 →<br/>
                AI 분석 → 영상 아이디어 → 파이프라인에 추가
              </div>
            </div>
          ) : (
            <>
                    ...
                    {items.map(item => (
                      <div key={item.id} style={{ background:T.c, border:`1px solid ${T.b}`, borderLeft:`3px solid ${stage.c}`, borderRadius:10, padding:14, marginBottom:8 }}>
                        ...
                        <div style={{ display:"flex", gap:5, flexWrap:"wrap" }}>
                          ...
                          <button onClick={() => rmPipe(item.id)} style={{ padding:"6px 10px", borderRadius:6, background:T.rd, border:`1px solid ${T.rb}`, color:T.r, fontSize:11, fontWeight:700, cursor:"pointer", fontFamily:T.m, marginLeft:"auto" }}>
                            ✕ 삭제
                          </button>
                        </div>
                      </div>
