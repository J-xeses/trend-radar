...
export default async function handler(req, res) {
  ...
  const { type, trend, video, videos, keyword, trendAnalysis, ytAnalysis, apiKey } = req.body;
...
{"score":9,"verdict":"지금 만들어야 합니다","common_keywords":["공통키워드1","공통키워드2","공통키워드3"],"blue_ocean":"트렌드는 뜨는데 유튜브에 아직 없는 블루오션 각도 한국어 구체적으로","final_title":"최종 확정 영상 제목 한국어 40자이내 (트렌드+유튜브 교차 최적화)","final_format":"숏폼 또는 롱폼 또는 튜토리얼","why_win":"이 영상이 잘 될 이유 2-3문장 한국어","risk":"주의할 점 한국어","upload_timing":"최적 업로드 타이밍 구체적으로 한국어","estimated_views":"예상 조회수 범위","thumbnail_concept":"썸네일 컨셉 한국어 구체적으로","hook_idea":"영상 첫 15초 후킹 아이디어 한국어"}`;

  } else {
    return res.status(400).json({ error: "type은 analyze, script, youtube_benchmark, cross_analysis 여야 합니다." });
  }
